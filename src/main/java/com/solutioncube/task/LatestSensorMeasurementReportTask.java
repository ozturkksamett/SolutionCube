...
public class LatestSensorMeasurementReportTask implements ITask {
	
	private static final Logger logger = LoggerFactory.getLogger(LatestSensorMeasurementReportTask.class);
	private final String COLLECTION_NAME = this.getClass().getSimpleName().substring(0, this.getClass().getSimpleName().length() - 4);
	private final String URI = "https://api.triomobil.com/facility/v1/reports/sensor/measurements/latest?_perPage=50";
